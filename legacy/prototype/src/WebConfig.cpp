...
<title>Camera Slider Config (Prototype)</title>
<style>
...
void handlePostConfig() {
  // Changing speeds, limits or step ratios underneath a running move would
  // corrupt motion already in flight.
  if (isBusy()) {
    server.send(409, "application/json",
                "{\"error\":\"a move is running; stop it before changing settings\"}");
    return;
  }
...
void handleReset() {
  if (isBusy()) {
    server.send(409, "application/json", "{\"error\":\"a move is running\"}");
    return;
  }
