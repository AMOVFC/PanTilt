...
// exactly governingTimeS to cover its shorter travel. Time dilation
// (t -> t/s) is exact: it stretches duration by s while leaving distance
// covered unchanged, scaling velocity by 1/s and acceleration by 1/s^2.
...
void ShotSequencer::begin(SlideAxis &slide, RotaryAxis &pan, RotaryAxis &tilt) {
  slide_ = &slide;
  pan_ = &pan;
  tilt_ = &tilt;
}
...
void ShotSequencer::beginMoveToKeyframe(const Keyframe &kf, uint32_t nowMs) {
  Serial.printf(
      "Shot: keyframe %u -> slide=%.1fmm pan=%.1fdeg tilt=%.1fdeg dur=%.2fs\n",
      static_cast<unsigned>(keyframeIndex_), kf.slideMm, kf.panDeg, kf.tiltDeg,
      kf.durationS);
  ...
  // allows). Every axis is then scaled to match.
...
void ShotSequencer::dispatchScurveSegment(uint8_t segment) {
  ...
  // All 3 axes share identical per-segment math but different concrete
  // types, so this can't be one loop over a common base without adding a
  // virtual interface just for this — three explicit blocks instead.
